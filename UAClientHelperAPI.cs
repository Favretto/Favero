...
namespace UAClientHelper
{
    public class UAClientHelperAPI
    {
        ...
        public void WriteValues(List<String> values, List<String> nodeIdStrings)
        {
            ...
            foreach (String str in nodeIdStrings)
            {
                ...
                catch //no base data type
                {
                    ...
                    else if (dataValue.Value.GetType().Name == "Single[]")
                    {
                        string[] arrString = values[nodeIdStrings.IndexOf(str)].Split(';');
                        Single[] arrInt = new Single[arrString.Length];

                        for (int i = 0; i < arrString.Length; i++)
                        {
                            arrInt[i] = Convert.ToSingle(arrString[i]);
                        }
                        variant = new Variant(arrInt);
                    }
                    else if (dataValue.Value.GetType().Name == "Boolean[]")
                    {
                        string[] arrString = values[nodeIdStrings.IndexOf(str)].Split(';');
                        bool[] arrBool = new bool[arrString.Length];

                        for (int i = 0; i < arrString.Length; i++)
                        {
                            arrBool[i] = Convert.ToBoolean(arrString[i]);
                        }
                        variant = new Variant(arrBool);
                    }
        ...
        private static ApplicationConfiguration CreateClientConfiguration()
        {
            ...
            configuration.ProductUri = "Teknosalliance.IndustrySupport";
